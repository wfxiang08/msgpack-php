...
// 定义标准格式的Zend Function: zif_msgpack_serialize
// 该function可以直接被export
//
...
// 定义ini的几个字段:
PHP_INI_BEGIN()
        STD_PHP_INI_BOOLEAN("msgpack.error_display", "1", PHP_INI_ALL, OnUpdateBool,
                            error_display, zend_msgpack_globals, msgpack_globals)

        // http://webcache.googleusercontent.com/search?q=cache:bvOShqYoeRwJ:gaofeihang.cn/archives/tag/msgpack+&cd=11&hl=en&ct=clnk&gl=us
        // 1. php将空的map反序列化成空对象，而不是空数组
        //    解决方案：php将php_only配置项置为false
        STD_PHP_INI_BOOLEAN("msgpack.php_only", "1", PHP_INI_ALL, OnUpdateBool,
                            php_only, zend_msgpack_globals, msgpack_globals)

        STD_PHP_INI_BOOLEAN("msgpack.illegal_key_insert", "0", PHP_INI_ALL, OnUpdateBool,
                            illegal_key_insert, zend_msgpack_globals, msgpack_globals)
        STD_PHP_INI_BOOLEAN("msgpack.use_str8_serialization", "1", PHP_INI_ALL, OnUpdateBool,
...
// 定义msgpack的函数
// 常见的: msgpack_pack   msgpack_serialize
...
static void msgpack_init_globals(zend_msgpack_globals *msgpack_globals) /* {{{ */ {
    ...
    // 默认只支持在php, 也许应该主动关闭 不要过度依赖php的特性??
...
static ZEND_MINIT_FUNCTION(msgpack) /* {{{ */ {
    ...
    REGISTER_INI_ENTRIES(); // 和 PHP_INI_BEGIN 相关
